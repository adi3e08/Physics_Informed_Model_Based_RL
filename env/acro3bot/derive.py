...
def derive():
    n = 3
    m1,l1,r1,I1,m2,l2,r2,I2,m3,l3,r3,I3,g = symbols('m1 l1 r1 I1 m2 l2 r2 I2 m3 l3 r3 I3 g')
    ...
    T3 = 0.5*m3*((l1*cos(q1)*q1dot+l2*cos(q1+q2)*(q1dot+q2dot)+r3*cos(q1+q2+q3)*(q1dot+q2dot+q3dot))**2 +\
                 (-l1*sin(q1)*q1dot-l2*sin(q1+q2)*(q1dot+q2dot)-r3*sin(q1+q2+q3)*(q1dot+q2dot+q3dot))**2) +\
         0.5*I3*((q1dot+q2dot+q3dot)**2)
    T = T1+T2+T3

    V = m1*g*r1*cos(q1)+m2*g*(l1*cos(q1)+r2*cos(q1+q2))+m3*g*(l1*cos(q1)+l2*cos(q1+q2)+r3*cos(q1+q2+q3))
    ...
    qddot = Minv*(Matrix([a1, 0, a3]) - C*qdot - G)
    ...
    D_s = F.jacobian(s)
    D_a = F.jacobian(Matrix([a1,a3]))
    # print(D)

    H_lambda = lambdify([tuple([m1,l1,r1,I1,m2,l2,r2,I2,m3,l3,r3,I3,g]+[q1,q2,q3,q1dot,q2dot,q3dot])],H,'numpy',cse=True)
    F_lambda = lambdify([tuple([m1,l1,r1,I1,m2,l2,r2,I2,m3,l3,r3,I3,g]+[q1,q2,q3,q1dot,q2dot,q3dot]+[a1, a3])],F,'numpy',cse=True)
    D_lambda = lambdify([tuple([m1,l1,r1,I1,m2,l2,r2,I2,m3,l3,r3,I3,g]+[q1,q2,q3,q1dot,q2dot,q3dot]+[a1, a3])],(D_s,D_a),'numpy',cse=True)
    
    with open("./env/acro3bot/dynamics.p", "wb") as outf:
        pickle.dump({'H':H_lambda,'F':F_lambda,'D':D_lambda}, outf)

    print("Done")

def check():
    inertial_params = [0.1,1.0,0.5,0.1/12,0.1,1.0,0.5,0.1/12,0.1,1.0,0.5,0.1/12,9.8]
    s = [1,1,1,0,0,0]
    a = [1,0,1]

    with open("./env/acro3bot/dynamics.p", "rb") as inf:
