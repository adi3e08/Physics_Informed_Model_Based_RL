...
def derive():
    n = 2
    m1,m2,l2,r2,I2,g = symbols('m1,m2,l2,r2,I2,g')
    ...
    qddot = Minv*(Matrix([a1, 0]) - C*qdot - G)
    ...
    H_lambda = lambdify([tuple([m1,m2,l2,r2,I2,g]+[q1,q2,q1dot,q2dot])],H,'numpy',cse=True)
    F_lambda = lambdify([tuple([m1,m2,l2,r2,I2,g]+[q1,q2,q1dot,q2dot]+[a1])],F,'numpy',cse=True)
    D_lambda = lambdify([tuple([m1,m2,l2,r2,I2,g]+[q1,q2,q1dot,q2dot]+[a1])],(D_s,D_a),'numpy',cse=True)
    
    with open("./env/cartpole/dynamics.p", "wb") as outf:
        pickle.dump({'H':H_lambda,'F':F_lambda,'D':D_lambda}, outf)

    print("Done")

def check():
    inertial_params = [1.0,0.1,1.0,0.5,0.1/12,9.8]
    s = [1,1,0,0]
    a = [1,0]

    with open("./env/cartpole/dynamics.p", "rb") as inf:
